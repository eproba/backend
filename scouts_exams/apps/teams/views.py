...
def calculate_user_score(user):
    ...
    """
    score = (
        Bank.objects.filter(accrual_deleted=False)
        .filter(user=user)
        .aggregate(Sum("accrual"))["accrual__sum"]
    )
...
def calculate_team_or_patrol_score(patrol_or_team):
    ...
    """
    return sum(
        [calculate_user_score(scout.user) for scout in patrol_or_team.scouts.all()]
    )
...
def view_team(request, team_id):
    ...
    patrols_scores = [
        (patrol, calculate_team_or_patrol_score(patrol))
        for patrol in Patrol.objects.filter(team=team)
    ]
...
def view_team_details(request, team_id):
    ...
    for week in weeks:
        taxes[week] = 0
        scores = (
            accurals.filter(year_week=week)
            .values("user", "accrual_type")
            .order_by()
            .annotate(Sum("accrual"))
        )
        for score in scores:
