...

{
    function initTinyMCE(el) {
        if (el.closest('.empty-form') === null) {  // Don't do empty inlines
            ...
            mce_conf.skin = window.matchMedia("(prefers-color-scheme: dark)").matches ? 'oxide-dark' : 'oxide';
            mce_conf.content_css = window.matchMedia("(prefers-color-scheme: dark)").matches ? 'dark' : 'default';
