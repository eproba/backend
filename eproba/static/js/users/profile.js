function update_device_list() {
    document.querySelectorAll('#notifications_devices > div > div > div').forEach(function (element) {
        const device_info = JSON.parse(element.dataset.deviceInfo);
        const os_version = (device_info['os'].version !== '10') ? device_info['os'].version : '10/11';
        const browser = device_info['browser'].toLowerCase();
        ...
        element.querySelector('.card-content .media .media-content .subtitle').textContent = device_info['browser'];
